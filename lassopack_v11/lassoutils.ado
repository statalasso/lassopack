*! lassoutils 1.1.01 08nov2018
*! lassopack package 1.1.01 08nov2018
*! authors aa/cbh/ms
...
program define _rlasso, rclass sortpreserve
	...
	*** defaults
	if `maxabsxflag' {
		...
		local corrnumber -1
		local maxupsiter 3		// since BCW say to iterate
	}
	if `c0'==0 {
									...
									real scalar noic)
{
		...
		lmax=max(lvec)
		lcount=cols(lvec)
		beta=luqrsolve(XX+lmax/2*diag(Ups2),Xy) // beta start	
		if (verbose==3) {
			printf("Initial beta:\n")
			beta'
		}
						...
						) 
{
	...
	if (args()<=12)	newseed = -1
	if (args()<=13) dotsflag = 0
	if (args()<=14) verbose = 0
	if (args()<=15) sqrtflag = 0
	...
	if (xdep==0) {									// X-independent
		if (lalt==0) {								// standard lambda
			lambda	= lassofactor * sqrt(d.n)*invnormal(1-gamma/gammad/(2*p))
		}
		else {										//  alternative lambda
			lambda	= lassofactor * sqrt(d.n) * sqrt(2 * log(2*p/(gamma/gammad)))
		}
		...
		lambda	= lassofactor * d.n * mm_quantile(sim,1,1-(gamma/gammad))
	}
				...
				real scalar prestdflag)
{
	...
	if (sqrtflag) {
		OUT	= RSqrtLasso(d,hetero,xdep,R,lassoUps,optTol,maxIter,zeroTol,maxUpsIter,UpsTol,verbose,c,c0,gamma,gammad,lambda0,lalt,corrnumber,pminus,nclust1,center,supscoreflag,ssnumsim,newseed,dotsflag)
	}
	else {
		OUT	= RLasso(d,hetero,xdep,R,lassoUps,optTol,maxIter,zeroTol,maxUpsIter,UpsTol,verbose,c,c0,gamma,gammad,lambda0,lalt,corrnumber,pminus,nclust1,center,sigma,supscoreflag,ssnumsim,newseed,dotsflag)
	}
							...
							)
{
	...
	if (sigma>0) {		// user-supplied sigma; assumes homoskedasticity
		...
		Ups		= d.sdvecpnp*sigma
		// note we use c and not c0
		lambda	= lambdaCalc(d,pminus,gamma,gammad,c,R,hetero,xdep,v,sigma,Ups,lalt,newseed,dotsflag,verbose,sqrtflag)
		betas	= DoLasso(d, Ups, Ups, lambda, lambda, verbose, optTol, maxIter, zeroTol, alpha)
		if (verbose>=1) {
		...
		Ups		= MakeScoreStdVec(d,v,s1,hetero,nclust1,center,0)
		...
		lambda	= lambdaCalc(d,pminus,gamma,gammad,c0,R,hetero,xdep,v,s1,Ups,lalt,newseed,dotsflag,verbose,sqrtflag)
		...
		do {
			...
			s0	= s1
			// obtain residuals; based on betas(Ups)
			if (lassoUps==1) {
			...
			lambda=lambdaCalc(d,pminus,gamma,gammad,c,R,hetero,xdep,v,s1,Ups,lalt,newseed,dotsflag,verbose,sqrtflag)
	...
	//               Ups = penalty loadings excluding rmse but standardizing;
	//               sUps = penalty loadings excluding rmse and standardization; =1 under homoskedasticity.
	betas.lambda0	= lambda
	betas.lambda	= lambda * s1
	betas.slambda	= lambda * s1 / d.ysdp
	betas.Ups		= betas.Ups / s1
				...
				real scalar prestdflag)
{
	...
	struct outputStruct scalar OUT
	OUT.supscore	= doSupScore(d, c, gamma, pminus, hetero, nclust1, center, verbose, R, newseed, dotsflag, ssiidflag)
	// Misc
	OUT.n			= d.n
	OUT.nclust		= d.nclust
	OUT.c			= c
...
//    object$pvalue <- sum(stat>object$supscore)/R
	...
	supscore_critvalue	=c*invnormal(1-gamma/(2*((d.p)-pminus)))
	
	res = (supscore, supscore_pvalue, supscore_critvalue, gamma)
	return(res)
}
							...
							)
{
	...
	if (hetero | d.nclust) {
		if (corrnumber>=0) {
			...
			v		= InitialResiduals(d,corrnumber)
			s1		= sqrt(mean(v:^2))
			Ups		= MakeScoreStdVec(d,v,s1,hetero,nclust1,center,1)
			...
			// Ups vector with zeros; cols corresponding to penalized vars updated below
			Ups		= J(1,cols(*d.X),0)
			if (d.nclust) {
				...
				Ups[1,(d.selindXp)]		= colmax(pmean)
			}
			else {
				// initial loadings = colmax(abs(X))
				Ups[1,(d.selindXp)]		= colmax(abs(centerpartial(d.Xp,d.mvecp,d.cons,d.Xnp,d.mvecnp,d.pihat)))
			}
			v		= .
			s1		= .
		}
	...
	if (lambda0) {
		...
		lambda=lambdaCalc(d,pminus,gamma,gammad,c,R,hetero,0,v,s1,Ups,lalt,newseed,dotsflag,verbose,sqrtflag)
	}
	...
	if ((hetero==0) & (d.nclust==0)) {
		...
		betas = DoSqrtLasso(d, Ups, lambda, verbose, optTol, maxIter, zeroTol)
		if (verbose>=1) {
			printf("Selected variables: %s\n\n",invtokens(betas.nameXSel'))
		}
		...
		do {								// start loop; always iterated at least once
			...
			// Ups update; last arg is sqrtflag
			...
			// xdep lambda update
			if (xdep) {
				lambda	= lambdaCalc(d,pminus,gamma,gammad,c,R,hetero,xdep,v,s1,Ups,lalt,newseed,dotsflag,verbose,1)
			}
		...
		// when iteration concludes, final estimated betas consistent with (have used) final Ups and lambda.
							...
							)
{
	...
	beta_init=luqrsolve(XX+lambda2/2*diag(Ups2),Xy)
	beta=beta_init
	if (verbose==2){
		w_old = beta
		k=1
	...
	// Separate blocks for lasso, ridge, elastic net.
	// Separate lambdas and penalty loadings for L1 and L2 norms to accommodate standardization.
	// If data are pre-standardized, then lambda=lambda2 and Ups=Ups2.
	// If standardization is on-the-fly and incorporated in penalty loadings,
	// then lambdas and penalty loadings for L1 and L2 norms are different.
	while (m < maxIter)
	{
		...
		if (verbose==2)
			{
				if (alpha==1) {
					...
					// elastic net
					fobj	= mean( (((*d.y):-d.ymvec) - ((*d.X):-d.mvec)*beta):^2 )	///
								+ lambda/n *alpha    *Ups*abs(beta)						///
								+ lambda2/n*(1-alpha)*Ups*(beta:^2)
	...
	if (alpha==1) {
		// lasso
		t.objfn		= (t.rmse)^2 + lambda/d.n * quadcross(Ups',abs(beta))
	}
	else if (alpha==0) {
		// ridge
		t.objfn		= (t.rmse)^2
		...
		// elastic net
		t.objfn		= (t.rmse)^2											///
						+ lambda/d.n  *alpha    *quadcross(Ups',abs(beta))	///
						+ lambda2/d.n *(1-alpha)*quadcross(Ups2',beta:^2)
	}
	if (verbose>=1) {
	...
	if (d.prestdflag==0) {
		// standardize
		t.objfn		= t.objfn  * 1/(d.ysd)^2
	}
	if (verbose>=1) {
		printf("Minimized objective function (standardized): %f\n",t.objfn)
	}
							...
							)
{
	...
	beta_init=luqrsolve(XX+lambda*diag(Ups),Xy)
	beta=beta_init
	...
	while (m < maxIter)
	{
		...
		if (ERRnorm>1e-10) {
			...
			dual = aaa'((*d.y):-(d.ymvec*d.cons))/n  - abs(lambda/n*Ups' - abs(((*d.X):-(d.mvec*d.cons))'aaa/n))'abs(beta)
		}
		else {
												...
												real scalar zeroTol)
{
		...
		lmax=max(lvec)
		lcount=cols(lvec)
		beta=luqrsolve(XX+lmax*diag(Ups),Xy) // beta start	
						...
						)
{
	...
	if (rows(t.betaAll)) {					// estimation results to insert
		...
		if (d.prestdflag & stdcoef==0) {
			...
			eUps		= eUps			:* d.prestdx				//  rlasso only; eUps and prestsdx both row vectors; Ups does not change
			rmse		= rmse			* d.prestdy
