	...
	foreach alphai of numlist `alpha' {
		...
		*** show warning if no unique lambda
		...
		if (`lunique'==0) {
			...
			di in smcl "{stata cvlasso, lopt}" 
			di as text "^ lse = largest lambda for which MSPE is within one standard error of the minimal MSPE."
			di as text "  Run model: " _c
			di in smcl "{stata cvlasso, lse}"
		}
		*
							 ...
							 real scalar sqrt)
{
	...
	st_view(X,.,xstring,touse)
		if (lmax<=0) {
		Ups = st_matrix(upsstr)
		if (cons) {
			Xy = quadcrossdev(X,mean(X),y,mean(y)):/(Ups')
		}
		else {
			Xy = quadcross(X,y):/(Ups')
		}
		if (sqrt==0) {
