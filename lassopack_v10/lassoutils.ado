							...
							)
{
	...
	if (d.cons==0) {
		// data are already demeaned or no constant in the model
		ynull = (*d.y) * 1/d.ysdp
	}
	else {
		ynull = ((*d.y):-(d.ymvec)) * 1/d.ysdp
	}
...
// Question: if homoskedastic, isn't E(ScoreStd)=E(ynull^2)E(x^2)=1?
	...
	// centerpartial(.) returns Xp demeaned and with Xnp partialled-out.
	hdm_ss			= 1/(d.n) * quadcolsum( (centerpartial(d.Xp,d.mvecp,d.cons,d.Xnp,d.mvecnp,d.pihat):/(d.sdvecp)) :* ynull )
...
//	lambda = c*sqrt(n)*invnormal(1-0.05/(2*p))
	...
	if ((hetero==0) & (d.nclust==0)) {
		...
		// Use sdvecpnp rather than unit vector since raw data are not pre-standardized.
		// NB: this is the vector of SDs after partialling and with zeros for unpenalized vars.
		Ups0 = d.sdvecpnp
