...
{bind:[{cmd:,}} {cmdab:a:lpha(}{it:real}{cmd:)}
{cmd:sqrt}
{cmdab:ada:ptive}
...
controls the {it:gamma} parameter of the EBIC. 
{it:gamma} needs to lie in the [0,1] interval.
{it:gamma}=0 is equivalent to the BIC. 
The default choice is {it:gamma}=1-log(n)/(2*log(p)). 
{p_end}
{synopt:{cmd:postest}} Used in combination with {cmd:lic()}. 
...
{p_end}
{synopt:{opt wnorm}†}
displays L1 norm of beta estimates weighted by penalty loadings, i.e., ||Ups*beta||(1) instead of ||beta||(1), which is the default. 
...
{p_end}
{synopt:Ups}
is a p by p diagonal matrix of predictor-specific penalty loadings. Note that {cmd:lasso2} treats Ups as a row vector.
{p_end}
{synopt:N}
...
These are given by (omitting dependence on lambda and alpha):
	...
	EBIC	= BIC + 2*{it:gamma}*{it:df}*log(p)
...
{pstd} 
The EBIC depends on an additional parameter, {it:gamma}, which can be controlled using 
{cmdab:ebicg:amma}{cmd:(}{it:real}{cmd:)}. 
{it:gamma}=0 is equivalent to the BIC.
We follow Chen & Chen ({helpb lasso2##Chen2008:2008}, p. 768) 
and use {it:gamma}=1-log(n)/(2*log(p))
as the default choice. 
An upper and lower threshold is applied to ensure that {it:gamma} lies in the [0,1] interval.
...
{pstd}
To store the estimation results of the selected model, add the {cmd:postest} option.{p_end}
{phang2}. {stata "lasso2, lic(ebic)"}{p_end}
{phang2}. {stata "ereturn list"}{p_end}
{phang2}. {stata "lasso2, lic(ebic) postest"}{p_end}
{phang2}. {stata "ereturn list"}{p_end}
...
Again, {cmd:postest} can be used to store results of the selected model.{p_end}
{phang2}. {stata "lasso2 lpsa lcavol lweight age lbph svi lcp gleason pgg45, lic(aic) postest"}{p_end}
...
{synopt:{cmd:e(Ups)}}row vector used penalty loadings{p_end}
