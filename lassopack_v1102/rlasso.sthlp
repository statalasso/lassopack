...
user-specified lambda0; overrides lasso default lambda = 2c*sqrt(N)*invnormal(1-(gamma/log(N))/(2*p))
...
defined as c*invnormal(1-gamma/(2p)).
...
the values of the (standardized) minimized objective functions
...
{synopt:{cmd:e(pmse)}}(standardized) minimized objective function (penalized mse, standard lasso only){p_end}
